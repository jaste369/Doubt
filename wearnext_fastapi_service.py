...
def gemini_classify_primary(image_path: str) -> dict[str, Any]:
    ...
    if not category_labels:
        empty_lists.append("categories")
    if not subcategory_labels:
        empty_lists.append("subcategories")
    if not colour_labels:
        empty_lists.append("colours")
    if not pattern_labels:
        empty_lists.append("patterns")
    if not fit_labels:
        empty_lists.append("fits")
    if not feature_labels:
        empty_lists.append("features")
    if empty_lists:
        return {
            "_warning": (
                f"Taxonomy lists are empty for: {', '.join(empty_lists)}. "
                "Check that WEARNEXT_BASE_DIR (or the individual *_JSON env vars) "
                "points to the directory containing your taxonomy JSON files."
            )
        }
    ...
    try:
        ...
        return {"_warning": f"Gemini returned non-JSON output: {e}. Raw text: {text[:300]!r}"}
    except Exception as e:
...
def _record_from_pred(image_path: str | None, image_name: str, pred: dict[str, Any], source: str = "llm") -> dict[str, Any]:
    warning = pred.get("_warning") if pred else None
    return {
...
def predict_bulk(files: list[UploadFile] = File(...)) -> dict[str, Any]:
    ...
    for file in files:
        ...
        try:
            pred = gemini_classify_primary(image_path)
            persistent_path = _persist_image(image_path, image_name)
            items.append(_record_from_pred(persistent_path, image_name, pred, source="llm"))
        finally:
            try:
                os.remove(image_path)
            except OSError:
...
def predict_and_export(
    ...
    try:
        ...
        return {
            "prediction": _record_from_pred(persistent_path, image_name, pred, source="llm"),
...
def startup_event() -> None:
    ...
    if not _get_api_key():
        print("  [ERROR] GOOGLE_API_KEY / GEMINI_API_KEY is NOT set.")
    else:
        print(f"  [OK]    Gemini model selected: {GEMINI_MODEL_NAME or 'NONE — check API key'}")

    print(f"  [INFO]  BASE_DIR    resolved to: {BASE_DIR}")
    print(f"  [INFO]  IMAGES_DIR  resolved to: {IMAGES_DIR}")
    ...
    all_ok = True
    for name, path in taxonomy_paths.items():
        count = taxonomy_counts[name]
        if not path.exists():
            print(f"  [ERROR] Missing file : {path}")
            all_ok = False
        elif count == 0:
            print(f"  [WARN]  Loaded 0 labels from {path} — check JSON structure/key names")
            all_ok = False
        else:
            print(f"  [OK]    {name:<16} {count} labels  ({path.name})")

    if all_ok:
        print("  [OK]    All taxonomy files loaded successfully.")
    else:
        print("  [ERROR] One or more taxonomy files missing or empty — predictions will fail.")
        print("          Set WEARNEXT_BASE_DIR or individual *_JSON env vars to fix this.")
